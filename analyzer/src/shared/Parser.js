import DeckData from '../assets/DeckData';
...
class DominionAnalyzer {
    ...
    parse(gameData) {
        try {
            ...
            // populate gdo.game
            _exports = {..._exports, ...this.populateGameName(gdo, gameDataLines)};

            // make Player objects in gdo.players[]
            _exports = {..._exports, ...this.populatePlayers(gdo, gameDataLines)};

            // look for each player's starting deck data above the turn logs, and populate them into turns[0]
            _exports = {..._exports, ...this.populateStartingDecks(gdo, gameDataLines)};

            // populate turn 1 into turns[1], turn 2 into turns[2] etc. for each player
            _exports = {..._exports, ...this.populateAllTurnsDecks(gdo, gameDataLines, _exports)};

            // populate turn 1 into turns[1], turn 2 into turns[2] etc. for each player
            _exports = {..._exports, ...this.populateAllTurnsPoints(gdo, _exports)};

            // compare our score calculations with those passed in through metadata if available
            _exports = {..._exports, ...this.checkMetadata(gdo, _exports)};
...
export default new DominionAnalyzer();